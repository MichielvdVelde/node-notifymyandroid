...
var Notifier = function(applicationName, apiKey) {
	...
	this.hasCallsRemaining = function() {
		return self.getRemainingCalls() == 0 && self.getRemainingTime() > 0;
	};
	...
	this.isValidKey = function(key, callback) {
		...
		if(!self.hasCallsRemaining()) return callback(new Error('Request limit reached. Limit will be reset within ' + remaining.resettimer + ' minutes'));
